"""VTMAdapter — SourceAdapter implementation for the VTM PostgreSQL database.

Connects to the VTM PostgreSQL instance via SQLAlchemy async engine and
...
class VTMAdapter:
    """Concrete :class:`~app.pipeline.extractor.SourceAdapter` for VTM.

    Connects to the VTM PostgreSQL database using a SQLAlchemy async engine
    and yields :class:`~app.models.ProductRecord` objects.

    Args:
        dsn: SQLAlchemy-compatible async DSN, e.g.
            ``"postgresql+asyncpg://user:pass@host:5432/vtm"``.
    """
    ...
    async def extract(
        ...
        """Yield :class:`~app.models.ProductRecord` objects from the VTM database.
